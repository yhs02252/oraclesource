...
--테이블 이름 작성 규칙
--1) 문자로 시작(한글 가능하나 사용하지 하지않음)
--2) 테이블 이름은 길이의 제한이 있음(30BYTE)
--3) 같은 소유자 소유의 테이블은 이름은 중복이 불가능함
--4) SQL 키워드는 사용 불가(SELECT, INSERT)
...
--5) SQL 키워드는 사용불가 (SELECT, INSERT)

--자료형
--문자 : VARCHAR2(길이), NVARCHAR2(길이), CHAR(길이), NCHAR(길이)
--      VAR : 가변(실제 저장된 값의 길이만큼만 사용) => VAR가 붙지 않는다면 명시된 만큼의 고정형 길이를 가진다
--   예) NAME VARCHAR2(10): 홍길동(9BYTE) 
--      NAME CHAR(10): 홍길동(10BYTE)
--      DB 버전에 따라 한글 문자 하나당 2BYTE 할당 OR 3BYTE 할당
--      NAME VARCHAR2(10): 홍길동전 => 들어갈 값의 크기가 크다고 오류남(12BYTE)
--          NVARCHAR2(10): 바이트 개념이 아닌 문자 길이 자체를 의미
--숫자 : NUMBER(전체자릿수, 소수점 자릿수)
--날짜 : DATE
--BLOB : 대용량 이진 데이터 저장 
--CLOB : 대용량 텍스트 데이터 저장
...
CREATE TABLE EMP_DDL(
    EMPNO NUMBER(4,0),
    ENAME VARCHAR2(10), -- 영어 10자 까지 가능 / 한글 3자까지 가능
    JOB VARCHAR2(9),
    MGR NUMBER(4,0),
    HIREDATE DATE,
    SAL NUMBER(7,2),
...
CREATE TABLE DETPT_DDL AS SELECT * FROM DEPT;

--DEPT 테이블의 열구조만 복사하여 새 테이블 생성
CREATE TABLE DETPT_TEMP AS SELECT * FROM DEPT WHERE 1<>1;
...
--새로운 열 추가
--EMP_DDL 에 새로운 열 추가 => PHONE NUMBER(HP : 010-1234-5637)
...
ALTER TABLE MEMBERTBL ADD BIGO VARCHAR(20);

ALTER TABLE MEMBERTBL MODIFY BIGO VARCHAR(30);

ALTER TABLE MEMBERTBL RENAME COLUMN BIGO TO REMARK;

INSERT INTO MEMBERTBL
VALUES ('hong1234', '홍길동', '서울시 구로구 개복동', '대한민국', 'hong123@naver.com', 25, null) ;
INSERT INTO MEMBERTBL
VALUES ('hong1235', '이승기', '서울시 강서구 화곡동', '대한민국', 'lee89@naver.com', 26, null) ;
INSERT INTO MEMBERTBL
VALUES ('hong1236', '강호동', '서울시 마포구 상수동', '대한민국', 'kang56@naver.com', 42, null) ;
INSERT INTO MEMBERTBL
VALUES ('hong1237', '이수근', '경기도 부천시', '대한민국', 'leesu@naver.com', 42, null) ;
INSERT INTO MEMBERTBL
VALUES ('hong1238', '서장훈', '서울시 강남구 대치동', '대한민국', 'seo568@google.com', 36, null) ;
INSERT INTO MEMBERTBL
VALUES ('hong1239', '김영철', '서울시 도봉구 도봉동', '대한민국', 'young@naver.com', 41, null) ;
INSERT INTO MEMBERTBL
VALUES ('hong1210', '김장훈', '서울시 노원구 노원1동', '대한민국', 'kim@naver.com', 48, null) ;
INSERT INTO MEMBERTBL
VALUES ('hong1211', '임창정', '서울시 양천구 신월동', '대한민국', 'limchang@naver.com', 45, null) ;
INSERT INTO MEMBERTBL
VALUES ('hong1212', '김종국', '서울시 강남구 도곡동', '대한민국', 'kimjong@naver.com', 44, null) ;
INSERT INTO MEMBERTBL
VALUES ('hong1213', '김범수', '경기도 일산동구 일산동', '대한민국', 'kim778@naver.com', 36, null) ;
INSERT INTO MEMBERTBL
VALUES ('hong1214', '김경호', '인천시 서구 가좌동', '대한민국', 'ho789@naver.com', 26, null) ;
INSERT INTO MEMBERTBL
VALUES ('hong1215', '민경훈', '경기도 수원시 수원1동', '대한민국', 'min@naver.com', 35, null) ;
INSERT INTO MEMBERTBL
VALUES ('hong1216', '바이브', '경기도 용인시 기홍구', '대한민국', 'vibe@naver.com', 33, null) ;
...
--뷰 : 가상 테이블
--1. 편리성 제공 : 복잡한 SELECT 문의 복잡도를 완화하기 위해
--              자주 활용하는 SELECT문을 뷰로 저장해 놓은 후 다른 SQL 구문에서 활용
--2. 보안성 확보 : 노출되면 안되는 컬럼을 제외하여 접근 허용

--뷰를 생성 할 수 있는 권한 부여 받기
--CREATE [OR REPLACE] VIEW 뷰 이름(열이름1, 열이름2...) AS (SELECT 구문)
